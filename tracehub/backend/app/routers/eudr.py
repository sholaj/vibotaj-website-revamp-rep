...
async def validate_shipment_eudr(
    ...
    # Sprint 14: Audit log EUDR validation
    is_compliant = status.get("is_compliant", False)
    audit_logger.log(
        action=AuditAction.EUDR_COMPLIANCE_PASS if is_compliant else AuditAction.EUDR_COMPLIANCE_FAIL,
        ...
        details={
            ...
            "risk_level": status.get("overall_risk_level"),
...
def _generate_pdf_report(report: dict) -> bytes:
    ...
    Sprint 14: Uses professional PDF generation with ReportLab.
    Returns properly formatted PDF with VIBOTAJ branding.
    """
    from ..services.pdf_generator import generate_eudr_compliance_report

    return generate_eudr_compliance_report(report)
...
async def verify_origin(
    ...
    if verification:
        if verification.farm_plot_identifier is not None:
            # Store in farm_name (primary identifier field)
            origin.farm_name = verification.farm_plot_identifier
        if verification.geolocation_lat is not None:
            origin.latitude = float(verification.geolocation_lat)
        if verification.geolocation_lng is not None:
            origin.longitude = float(verification.geolocation_lng)
        ...
        if verification.production_start_date is not None:
            origin.production_date = verification.production_start_date
        if verification.production_end_date is not None:
            origin.harvest_date = verification.production_end_date
        if verification.deforestation_free_statement is not None:
        ...
        if verification.supplier_attestation_reference is not None:
            origin.due_diligence_statement_ref = verification.supplier_attestation_reference

        # Mark as verified
        origin.verified_at = datetime.utcnow()
        origin.verified_by = current_user.id  # Use UUID, not username
...
async def get_origin_risk(
    ...
    # Sprint 14: Audit log deforestation risk check
    audit_logger.log(
        action=AuditAction.EUDR_DEFORESTATION_CHECK,
        user_id=str(current_user.id),
        username=current_user.username,
        organization_id=str(current_user.organization_id),
        resource_type="origin",
        resource_id=str(origin_id),
        success=True,
        details={
            "risk_level": risk.get("risk_level"),
            "country": origin.country,
            "source": risk.get("source", "country_baseline"),
            "forest_loss_detected": risk.get("forest_loss_detected"),
        },
        db=db
    )
