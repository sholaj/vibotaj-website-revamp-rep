...
export default function CreateShipmentModal({
  ...
  onClose,
  onSuccess,
  organizationId: _organizationId, // Reserved for future multi-tenancy filtering
  ...
  // Load buyer organizations when modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  const loadBuyerOrganizations = async () => {
    ...
    try {
      ...
      // Don't show error - buyer selection is optional
    } finally {
  ...
  const generateReference = () => {
    ...
    setReference(`VIBO-${year}-${paddedNum}`)
    setReferenceError(null)
  }
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label htmlFor="vesselName" className="block text-sm font-medium text-gray-700 mb-1">
                  Vessel Name <span className="text-gray-400">(optional)</span>
                </label>
                <input
                  id="vesselName"
                  type="text"
              ...
              <div>
                <label htmlFor="buyerOrg" className="block text-sm font-medium text-gray-700 mb-1">
                  Buyer Organization <span className="text-gray-400">(optional)</span>
                </label>
                <select
                  id="buyerOrg"
                  value={buyerOrgId}
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  <label htmlFor="isHistorical" className="font-medium text-gray-700">
                    Historical Shipment
                  </label>
                  <p className="text-gray-500">
                    Check this if adding a past shipment for record keeping
                  </p>
                </div>
              </div>
